defmodule Incant.Service.Page do
  ...
  @derive {Jason.Encoder, only: [:rows, :page, :page_size, :total, :total_pages, :meta, :error]}
  defstruct rows: [], page: 1, page_size: 25, total: 0, total_pages: 1, meta: %{}, error: nil
  ...
  @spec from_resource_page(map(), map(), map()) :: t()
  def from_resource_page(page, resource, context \\ %{}) when is_map(page) do
    %__MODULE__{
      rows:
        page
        |> Map.get(:rows, [])
        |> Enum.map(&Incant.Service.Row.from_resource(resource, &1, context)),
